...
#include "lxveos_hidmap.h"  // ascii_to_hid / named_key (US-layout keystroke map, host-tested)
...
// The US-layout keystroke map (ascii_to_hid / named_key) lives in the host-tested lxveos_hidmap
// component (tests/host_c/test_hidmap.c) — a wrong keycode types the wrong key, so it's verified
// exhaustively off-target. See lxveos_hidmap.h.
