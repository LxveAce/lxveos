...
| `pwnwatch [seconds] [channel]` | passive Pwnagotchi-presence detector — flag beacons from the fixed Pwnagotchi grid MAC `de:ad:be:ef:de:ad` and decode the JSON identity (name + handshake count) carried in the beacon SSID; ported from ESP32 Marauder's "Detect Pwnagotchi" (see `CREDITS.md`); channel-hops by default or dwell on a channel `1-13`; listen only, sends nothing |
