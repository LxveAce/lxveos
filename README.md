...
| `capture [seconds] [channel]` | passive EAPOL/PMKID capture — detect WPA 4-way-handshake messages and emit hashcat-22000 lines: a `WPA*01` line per RSN PMKID **and** a `WPA*02` line per captured M1+M2 handshake (MIC + EAPOL, replay-matched) — both feed the Cyber Controller crack pipeline; channel-hops by default, or pass a channel `1-13` to dwell on a known AP's channel; listen only, never forces a handshake |
