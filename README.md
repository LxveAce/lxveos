...
| `apaudit` | passive AP security-posture auditor (custom) — grade every AP's encryption and flag the weak ones: open (no encryption), WEP (broken cipher), legacy WPA (deprecated TKIP); prints a per-grade tally (open/WEP/WPA/WPA2/WPA3) + hidden-SSID count over one passive scan; listen only, sends nothing |
