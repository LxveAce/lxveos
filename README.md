...
**One multi-board ESP32 security firmware. By LxveLabs, built by LxveAce.**

LxveOS is one MIT-licensed [ESP-IDF](https://docs.espressif.com/projects/esp-idf/) codebase that runs on many
boards. Each board gets its own variant tuned to what that board does well, and features turn on only where the
hardware supports them. It builds on the open ESP32 security firmware that came before it (Marauder, ESP32-DIV,
GhostESP, Bruce, and others), with credit and without competing with them: permissively-licensed code is reused
directly, and copyleft work is learned from clean-room (ideas, not code). See [`CREDITS.md`](CREDITS.md).
...
> serial control console and the full operation suite (passive Wi-Fi/BLE recon, defensive detectors, wardrive CSV logging,
...
> pending. Read "exists in code / CI-built" as different from "validated on silicon": nothing here claims to work on a
> device until it's bench-validated.
> **Authorized, lawful security research and education only.** See [`RESPONSIBLE-USE.md`](RESPONSIBLE-USE.md).
...
## Design
The implementation is here, and [`cyd_boards.json`](cyd_boards.json) is the single source of truth for board
...
## Core decisions
- Built on ESP-IDF v6.0.x with CMake; the core is MIT-licensed. The UI stack is `esp_lcd` + `esp_lvgl_port` + LVGL 9
  (LovyanGFX as a wrapped fallback backend), and the board interface follows the Espressif `esp-bsp` contract.
- One codebase, board support as data: each board is a row in `cyd_boards.json`, and `scripts/gen_board_configs.py`
  turns it into the per-board build inputs under `boards/`. Adding a board is one JSON edit.
- The flagship case: the ILI9341 (1-USB) vs ST7789 (2-USB) CYD panels are electrically identical, so
  chip/features/partitions are set at compile time and the panel driver is chosen at boot by probing the panel ID
  register. (Persisting the probed result to NVS so later boots skip the probe is planned; M0 caches the build-time
  default.)
...
Because this is security tooling, the console stays locked on first boot until the operator types `agree` to accept the
authorized-use terms. The acceptance is stored in NVS, so it's a one-time gate per unit rather than a per-boot prompt.
...
| `wardrive` | passive Wi-Fi wardrive CSV export: one passive scan produces a machine-importable `bssid,ssid,channel,rssi,auth,hidden` CSV (SSID quoted/escaped) for a host mapping/inventory tool; GPS-less (the host adds coordinates); listen only |
| `blewardrive [seconds]` | passive BLE wardrive CSV export: one passive GAP-observe scan produces a machine-importable `addr,addr_type,name,rssi,vendor,tracker` CSV (name quoted/escaped) for a host mapping/inventory tool; the BLE mirror of `wardrive`; GPS-less (the host adds coordinates); listen only |
| `airspace [ble_seconds]` | airspace occupancy summary (custom): one passive Wi-Fi scan + (when BLE is active) a short passive BLE observe give a quick "what's around me": AP count with open / WPS-exposed / hidden splits, and BLE advertiser count with known-tracker count; emits a single `LXVEOS/1 snapshot` event for the Cyber Controller dashboard; listen only, sends nothing |
| `watch add <mac> [label]` · `watch del <mac>` · `watch list` · `watch clear` · `watch scan [ble_seconds]` | **defense**: target-address watchlist (custom): register BSSIDs / BLE addresses of interest (`aa:bb:cc:dd:ee:ff`, optional label, up to 16, kept across reboots), then `watch scan` runs one passive Wi-Fi + (when BLE is active) BLE sweep and flags any listed target that's on the air right now with its band + RSSI, emitting one `LXVEOS/1 alert kind=watch` per hit for the Cyber Controller dashboard; listen only, sends nothing |
...
| `meta [seconds]` | passive Meta / Ray-Ban Meta glasses + Oculus detector: flag advertisers whose manufacturer company ID, an advertised service-class UUID, or a service-DATA UUID is in the Meta set (covering all three surfaces catches Meta's `0xFD5F` anchor), with an Apple/Samsung/Microsoft deny-list checked first (blocked wins, so a popup-flood payload can't false-match); ported from ESP32 Marauder's "Meta Detect" (see `CREDITS.md`); listen only |
| `skimmer [seconds]` | passive card-skimmer heuristic: flag advertisers whose local name is exactly a default HC-0x BT-serial module name (`HC-03`/`HC-05`/`HC-06`), the stock modules cheap skimmers reuse; ported from ESP32 Marauder's "Detect Card Skimmers" (see `CREDITS.md`). Narrow, possible-only: it also flags legit hobby HC-0x modules, and being BLE-only it won't see classic-BT-only skimmers, so treat a hit as "verify in person" rather than a confirmed skimmer; listen only |
| `flock [seconds]` | **experimental**: passive Flock Safety camera heuristic; flag advertisers carrying Flock's XUNTONG manufacturer ID (`0x09C8`), tiered `likely` (a confirming Flock name: `Penguin-<10 digits>` / `FS Ext Battery` / a bare 10-digit serial) vs `possible` (a nameless XUNTONG advert); ported from ESP32 Marauder's "Flock Sniff" (see `CREDITS.md`). LxveOS carries only the specific XUNTONG signal. Marauder's broad 27-entry OUI list (incl. a `CC:CC:CC` placeholder) and generic Wi-Fi SSID substrings (`flock`/`penguin`/`pigvision`) are deliberately not carried, because they false-match ordinary gear. A hit is a lead to verify in person, never a confident camera ID; listen only |
| `surveil [seconds]` | **defense (custom)**: counter-surveillance / anti-stalking sweep: one passive BLE scan that folds every passive detector into a single "what surveillance gear is near me?" answer: item-trackers, Flock cameras, Meta / Ray-Ban glasses (wearable camera + mic), Flipper Zeros, and possible card-skimmers, each with a per-device category label and a category tally; emits one `LXVEOS/1 alert kind=surveil` line for the Cyber Controller dashboard. Skimmer/Flock rows stay heuristic (verify in person). Listen only |
| `blehid [seconds]` | **defense**: flag nearby BLE HID devices (advertising the 0x1812 HID service or a HID GAP appearance) as rogue-keyboard / injector candidates; listen only |
...
| `ir recv <rx_gpio> [s] \| send <tx_gpio> \| show` | IR capture + replay via RMT (universal-remote); `send` transmits a captured frame; `recv`/`show` also decode the capture when it's NEC (incl. the held-button repeat code + 16-bit extended addressing) Sony SIRC (12/15/20-bit), or Philips RC5/RC5X (14-bit Manchester), printing `proto addr=… cmd=…`. An unrecognized signal (e.g. RC6 Manchester, A/C remotes) still records + replays as raw symbols. Needs an IR receiver/emitter |
| `subghz begin <sclk> <miso> <mosi> <cs> \| rssi <mhz> \| capture <gdo0> <mhz> [s] \| decode \| replay <gdo0> \| end` | CC1101 sub-GHz: RSSI + OOK `capture` (receive). `decode` bit-decodes the captured signal — it recovers the `0`/`1` stream from the PWM encoding common 315/433 MHz OOK remotes share (short/long pulse pairs, long-gap frame sync), printing the decoded bits; the OOK bit-decoder is pure + host-tested (CI-green), while the CC1101 RF itself is an add-on module (on-radio capture/replay is hardware-gated). `replay` is arm-gated (offensive OOK re-emit) |
| `nrf24 begin <sck> <miso> <mosi> <csn> <ce> \| scan [sweeps] \| sniff \| mousejack <text> \| end` | nRF24L01+ 2.4 GHz: channel scan + address sniff (receive) plus arm-gated `mousejack` keystroke injection. Add-on module |
...
dependency pins in `scripts/expected_deps.txt` vs the freshly resolved lock, failing on drift) and
`scripts/size_report.py` (app image vs its partition slot, failing on overflow and warning over a soft budget);
IRAM/DRAM overflow already fails the build at link time. It's currently green. The one build-side gate still TODO is
