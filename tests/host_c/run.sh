...
# --- Wi-Fi EAPOL/PMKID hashcat-22000 line formatter: dependency-free, no stubs needed ---
"$CC" -std=gnu11 -O1 -Wall \
    -I"$root/components/lxveos_wifi/include" \
    "$here/test_wifi_eapol.c" "$root/components/lxveos_wifi/src/lxveos_wifi_eapol.c" -o "$out"
"$out"

# --- Wi-Fi BSSID->ESSID map + de-cloak upsert: dependency-free, no stubs needed ---
"$CC" -std=gnu11 -O1 -Wall \
    -I"$root/components/lxveos_wifi/include" \
    "$here/test_wifi_essidmap.c" "$root/components/lxveos_wifi/src/lxveos_wifi_essidmap.c" -o "$out"
"$out"

# --- Wi-Fi scan analytics (evil-twin classification): dependency-free, no stubs needed ---
"$CC" -std=gnu11 -O1 -Wall \
    -I"$root/components/lxveos_wifi/include" \
...
# --- config export/import escaped-text codec: dependency-free, no stubs needed ---
"$CC" -std=gnu11 -O1 -Wall \
    -I"$root/components/lxveos_cfg/include" \
