...
LxveOS builds on the open ESP32 security-firmware community, with proper credit and without competing.
Ideas and concepts are free to learn from (they aren't copyrightable); only permissively-licensed code is
reused directly. The full per-project map is in [`THIRD-PARTY-LICENSES.md`](THIRD-PARTY-LICENSES.md).
...
- **ESP32 Marauder**, Just Call Me Koko (MIT), *Copyright (c) 2020 Just Call Me Koko*: Wi-Fi scan/monitor,
  ...
  glasses ("Meta Detect"), the card-skimmer default-name heuristic ("Detect Card Skimmers"), and the Flock Safety
  camera heuristic ("Flock Sniff"; LxveOS carries only the specific XUNTONG-mfg BLE signal, not the broad-OUI /
  SSID-substring paths). Full MIT license text retained in `THIRD-PARTY-LICENSES.md`.
...
- **GhostESP** (GPL-3.0), **Bruce** (AGPL-3.0), **M5Stick-NEMO** (GPL), **minigotchi** (GPL): UX, detectors,
  scripting, and pet-mode concepts reimplemented independently. **Spacehuhn deauther** (CC BY-NC): ethos only.

## Stance
- No endorsement or trademarks: "Marauder", "GhostESP", "Bruce", "NEMO" etc. are used only for factual
